...
namespace Unnamed
{
    public class Player : Actor
    {
        ...
        public const int PlayerWidth = 8;
        ...
        private Kinematic grappledSolid;
        ...
        public Player(Vector2 position) : base(position, new AABBCollider(Vector2.Zero, PlayerWidth, PlayerHeight), new Sprite(Color.White))
        {
            Platformer.Player = this;
        ...
        private void ThrowRope()
        {
            ...
            if (CheckForSwingingPoint(out Kinematic determinedGrappledSolid, out float distance))
            {
            ...
            bool CheckForSwingingPoint(out Kinematic swingingPoint, out float distance)
            {
                Kinematic determinedGrappledSolid = null;
                Kinematic reserveGrappledSolid = null;
                distance = float.PositiveInfinity;
                float reserveDistance = float.PositiveInfinity;

                foreach (Kinematic g in SwingingPoint.SwingingPoints)
                {
            ...
            void ActOnSwing(Kinematic grappledSolid)
            {
